		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="card">
					...
					<form method="post" id="add_candidate_form">
						<div class="card-body">
								...
								<select  class="form-control" name="candidate" id="candidate" required>
									...
									if($student->num_rows() > 0 ):
										foreach($student->result_array() as $row):
											echo '
												<option value="'.$row['student_id'].'">'.ucwords($row['last_name'] .', '.$row['first_name'] . ' ' . $row['middle_name']).'</option>
											';
										endforeach;
									endif;
									?>
								...
								<select  class="form-control" name="position" id="position" required>
									...
									if($position->num_rows() > 0 ):
										foreach($position->result_array() as $row):
											echo '
												<option value="'.$row['position_id'].'">'.ucwords($row['position']).'</option>
											';
										endforeach;
									endif;
									?>
								...
								<select  class="form-control" name="organization" id="organization" required>	
									...
									if($organization->num_rows() > 0 ):
										foreach($organization->result_array() as $row):
											echo '
												<option value="'.$row['organization_id'].'">'.ucwords($row['organization']).'</option>
											';
										endforeach;
									endif;
									?>
